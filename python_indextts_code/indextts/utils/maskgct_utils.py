...
def build_semantic_model(path_='./models/tts/maskgct/ckpt/wav2vec2bert_stats.pt'):
    import glob
    # 查找 w2v-bert-2.0 模型的最新快照目录
    w2v_snapshot_dirs = glob.glob(str(CACHE_DIR / "models--facebook--w2v-bert-2.0" / "snapshots" / "*"))
    if not w2v_snapshot_dirs:
        # 如果没有找到本地快照，尝试从缓存目录下载
        semantic_model = Wav2Vec2BertModel.from_pretrained(
            "facebook/w2v-bert-2.0", cache_dir=str(CACHE_DIR)
        )
    else:
        # 选择最新的快照目录
        local_w2v_dir = max(w2v_snapshot_dirs, key=os.path.getmtime)
        semantic_model = Wav2Vec2BertModel.from_pretrained(
            str(local_w2v_dir), local_files_only=True
        )
    semantic_model.eval()
