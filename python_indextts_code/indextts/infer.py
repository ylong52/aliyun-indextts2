...
class IndexTTS:
    def __init__(
        ...
        if "gpt" not in self.cfg:
            raise ValueError(
                f"Invalid IndexTTS config: missing `gpt` section in {cfg_path!r}. "
                "Please ensure `cfg_path` points to the correct checkpoints config (e.g. `checkpoints/config.yaml`) "
