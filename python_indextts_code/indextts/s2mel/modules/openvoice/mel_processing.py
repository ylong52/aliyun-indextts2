...
def spectrogram_torch(y, n_fft, sampling_rate, hop_size, win_size, center=False):
    ...
    # 修复 padding 问题：根据 tensor 维度调整 padding
    pad_size = int((n_fft - hop_size) / 2)
    if len(y.shape) == 2:
        # 2D: (batch, length) -> 添加 channel 维度后 padding
        y = y.unsqueeze(1)  # (batch, 1, length)
        y = torch.nn.functional.pad(y, (pad_size, pad_size), mode="reflect")
        y = y.squeeze(1)
    elif len(y.shape) == 3:
        # 3D: (batch, channel, length) -> padding 需要是 4 个值 (left, right, top, bottom)
        y = torch.nn.functional.pad(y, (pad_size, pad_size, 0, 0), mode="reflect")
    else:
        # 其他情况，使用原始方法
        y_padded = y.unsqueeze(1) if len(y.shape) == 2 else y
        if len(y_padded.shape) == 3:
            y = torch.nn.functional.pad(y_padded, (pad_size, pad_size), mode="reflect")
            y = y.squeeze(1) if len(y.shape) == 2 else y
        else:
            y = torch.nn.functional.pad(y_padded, (pad_size, pad_size, 0, 0), mode="reflect")
