...
try:
    ...
    def _crop_past_key_values(model, past_key_values, new_cache_size):
        ...
        if isinstance(past_key_values, tuple):
            if len(past_key_values) > 0 and isinstance(past_key_values[0], tuple):
                # 格式: ((key, value), ...)
                return tuple(
                    tuple(kv[..., :new_cache_size, :] if kv is not None else None for kv in layer)
    ...
    def _prepare_attention_mask(kwargs, seq_length, is_encoder_decoder):
        """准备 attention_mask"""
        ...
        if attention_mask.shape[-1] != seq_length:
            # 扩展或裁剪 attention_mask
            if attention_mask.shape[-1] < seq_length:
                # 扩展
                padding = torch.ones(
                    attention_mask.shape[:-1] + (seq_length - attention_mask.shape[-1],),
    ...
    def _prepare_token_type_ids(kwargs, seq_length):
        """准备 token_type_ids"""
        ...
        if token_type_ids.shape[-1] != seq_length:
            # 扩展或裁剪 token_type_ids
            if token_type_ids.shape[-1] < seq_length:
                # 扩展（使用最后一个值）
                last_value = token_type_ids[..., -1:]
                padding = last_value.repeat(seq_length - token_type_ids.shape[-1])
...
try:
    ...
    # 如果这些映射不可用，提供默认的空字典
    # 这些映射用于缓存实现，如果不可用，将使用默认的 DynamicCache
    NEED_SETUP_CACHE_CLASSES_MAPPING = {}
    QUANT_BACKEND_CLASSES_MAPPING = {}
...
# 尝试导入 QuantizedCacheConfig，如果不存在则使用 None
try:
    from transformers.cache_utils import QuantizedCacheConfig
except ImportError:
    # 如果 QuantizedCacheConfig 不存在，创建一个占位类
...
class GenerationMixin:
    ...
    def _get_logits_processor(
        ...
        # 检查 forced_decoder_ids 属性是否存在（某些版本的 transformers 可能没有这个属性）
    ...
    def _get_cache(
        ...
        """
        # 如果映射不可用，使用默认的 DynamicCache
        if cache_implementation in NEED_SETUP_CACHE_CLASSES_MAPPING:
            cache_cls: Cache = NEED_SETUP_CACHE_CLASSES_MAPPING[cache_implementation]
        else:
            # 如果映射为空（某些版本的transformers不支持），使用默认的 DynamicCache
            cache_cls: Cache = DynamicCache
    ...
    def _prepare_cache_for_generation(
        ...
        if generation_config.cache_implementation is not None:
            if generation_config.cache_implementation in NEED_SETUP_CACHE_CLASSES_MAPPING:
                ...
                # 如果映射不可用，抛出错误（因为量化缓存需要特定的后端支持）
                if cache_config.backend not in QUANT_BACKEND_CLASSES_MAPPING:
                    raise ValueError(
                        f"Quantized cache backend '{cache_config.backend}' is not supported. "
                        "This may be due to an incompatible transformers version."
                    )
